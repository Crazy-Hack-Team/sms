#MR.H4N74M GANS
#RECODE NYA JAGO JUGA CUK WKWKWK
...
def only():
    ...
    if msg == '':
        ...
        get = br.submit().read()
        if 'SMS Geratis Dikirim' in str(get):
            print tutup + '[' + lime + '+' + tutup + '] Terkirim : ' + lime + str(no)
            p = raw_input(tutup + '[' + lime + '?' + tutup + '] Kirim Lagi? [y/n] : ' + lime)
            if p == 'y':
                os.system('python2 sms.py')
            else:
                exit(tutup + '[' + merah + '!' + tutup + '] Exit')
        else:
            print tutup + '[' + lime + '+' + tutup + '] Berhasil Mengirim Karna Mr.H4N74M Gans  : ' + lime + str(no)
            p = raw_input(tutup + '[' + lime + '?' + tutup + '] Kirim Lagi? [y/n] : ' + lime)
            if p == 'y':
                os.system('python2 sms.py')
            else:
...
if __name__ == '__main__':
    try:
        ...
        os.system('xdg-open https://github.com/Crazy-Hack-Team')
        ko = 'Free SMS by Mr.H4N74M'
        s = open('parker', 'w')
        s.write(ko)
        s.close()
        only()